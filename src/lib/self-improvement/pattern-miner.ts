 ...
 */
/**
 * Optional Global Workspace for live broadcasting of discovered patterns
 * (Phase 6: Buddhist Dependent Origination — no agent is an island).
 */
interface PatternMinerOptions {
  /** When provided, broadcasts discovered patterns to the workspace in real-time. */
...
export async function mineToolPatterns(
  ...
  // Phase 6: Broadcast discovered patterns to the Global Workspace
  if (options?.workspace && patterns.length > 0) {
    for (const pattern of patterns.slice(0, 5)) { // limit broadcasts to top 5
      options.workspace.broadcastPattern(
        `pattern-miner:${agentId}`,
        `Tool sequence: ${pattern.toolSequence.join(' → ')} (${pattern.support} occurrences, ${Math.round(pattern.avgSuccessRate * 100)}% success)`,
        Math.min(0.8, pattern.support / 10), // salience scales with support
      );
    }
  }
