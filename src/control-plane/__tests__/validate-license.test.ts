...
describe('validateLicenseKey', () => {
  ...
  it('network error throws LicenseValidationError with network kind', async () => {
    globalThis.fetch = vi.fn().mockRejectedValue(new Error('Network failure'));
    await expect(validateLicenseKey('key-123', CLOUD_URL)).rejects.toThrow('Could not reach the cloud');
  });
