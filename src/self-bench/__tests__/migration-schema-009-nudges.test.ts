...
const EXPECTED = ['agent_workforce_nudges'];

describe('MigrationSchema009NudgesExperiment (auto-generated)', () => {
  const exp = new MigrationSchema009NudgesExperiment();
  ...
  it('evidence carries migration_file and the expected list', async () => {
    const rows = [{ name: 'agent_workforce_nudges' }];
    const result = await exp.probe(makeCtx(rows));
    const ev = result.evidence as { migration_file: string; expected_tables: string[] };
    expect(ev.migration_file).toBe('009-nudges.sql');
