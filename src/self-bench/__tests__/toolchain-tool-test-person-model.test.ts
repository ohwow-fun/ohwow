...
describe('ToolchainToolTestPersonModelExperiment (auto-generated)', () => {
  const exp = new ToolchainToolTestPersonModelExperiment();
  ...
  it('returns pass when command exits 0', async () => {
    ...
    expect(ev.command).toBe('npx vitest run src/orchestrator/tools/__tests__/person-model.test.ts');
  });
