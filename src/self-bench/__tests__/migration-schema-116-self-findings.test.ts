...
const EXPECTED = ['self_findings'];

describe('MigrationSchema116SelfFindingsExperiment (auto-generated)', () => {
  const exp = new MigrationSchema116SelfFindingsExperiment();
  ...
  it('evidence carries migration_file and the expected list', async () => {
    const rows = [{ name: 'self_findings' }];
    const result = await exp.probe(makeCtx(rows));
    const ev = result.evidence as { migration_file: string; expected_tables: string[] };
    expect(ev.migration_file).toBe('116-self-findings.sql');
