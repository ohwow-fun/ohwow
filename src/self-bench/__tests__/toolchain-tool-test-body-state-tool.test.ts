...
import { ToolchainToolTestBodyStateToolExperiment } from '../experiments/toolchain-tool-test-body-state-tool.js';
...
describe('ToolchainToolTestBodyStateToolExperiment (auto-generated)', () => {
  const exp = new ToolchainToolTestBodyStateToolExperiment();
  ...
  it('returns pass when command exits 0', async () => {
    ...
    expect(ev.command).toBe('npx vitest run src/orchestrator/tools/__tests__/body-state-tool.test.ts');
  });
