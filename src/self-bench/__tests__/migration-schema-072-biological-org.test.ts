...
const EXPECTED = ['agent_synapses'];

describe('MigrationSchema072BiologicalOrgExperiment (auto-generated)', () => {
  const exp = new MigrationSchema072BiologicalOrgExperiment();
  ...
  it('evidence carries migration_file and the expected list', async () => {
    const rows = [{ name: 'agent_synapses' }];
    const result = await exp.probe(makeCtx(rows));
    const ev = result.evidence as { migration_file: string; expected_tables: string[] };
    expect(ev.migration_file).toBe('072-biological-org.sql');
