...
import { ToolchainToolTestResyncExperiment } from '../experiments/toolchain-tool-test-resync.js';
...
describe('ToolchainToolTestResyncExperiment (auto-generated)', () => {
  const exp = new ToolchainToolTestResyncExperiment();
  ...
  it('returns pass when command exits 0', async () => {
    ...
    expect(ev.command).toBe('npx vitest run src/orchestrator/tools/__tests__/resync.test.ts');
  });
