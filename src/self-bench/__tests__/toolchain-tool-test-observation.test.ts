...
describe('ToolchainToolTestObservationExperiment (auto-generated)', () => {
  const exp = new ToolchainToolTestObservationExperiment();
  ...
  it('returns pass when command exits 0', async () => {
    ...
    expect(ev.command).toBe('npx vitest run src/orchestrator/tools/__tests__/observation.test.ts');
  });
