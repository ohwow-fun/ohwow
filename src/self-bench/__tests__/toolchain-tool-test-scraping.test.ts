...
import { ToolchainToolTestScrapingExperiment } from '../experiments/toolchain-tool-test-scraping.js';
...
describe('ToolchainToolTestScrapingExperiment (auto-generated)', () => {
  const exp = new ToolchainToolTestScrapingExperiment();
  ...
  it('returns pass when command exits 0', async () => {
    ...
    expect(ev.command).toBe('npx vitest run src/orchestrator/tools/__tests__/scraping.test.ts');
  });
