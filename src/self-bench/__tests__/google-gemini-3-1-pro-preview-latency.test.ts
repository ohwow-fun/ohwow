...
import { GoogleGemini31ProPreviewLatencyExperiment } from '../experiments/google-gemini-3-1-pro-preview-latency.js';
...
describe('GoogleGemini31ProPreviewLatencyExperiment (auto-generated)', () => {
  const exp = new GoogleGemini31ProPreviewLatencyExperiment();

  it('returns warning when samples < min_samples', async () => {
    const rows = Array.from({ length: 5 }, () => ({ latency_ms: 100 }));
  ...
  it('returns pass when p50 is below warn threshold', async () => {
    const rows = Array.from({ length: 6 }, () => ({ latency_ms: 26115 }));
    const result = await exp.probe(makeCtx(rows));
    expect(exp.judge(result, [])).toBe('pass');
  });

  it('returns warning when p50 crosses warn threshold', async () => {
    const rows = Array.from({ length: 6 }, () => ({ latency_ms: 52231 }));
    const result = await exp.probe(makeCtx(rows));
    const ev = result.evidence as { p50_latency_ms: number };
    expect(ev.p50_latency_ms).toBeGreaterThanOrEqual(52230);
    expect(exp.judge(result, [])).toBe('warning');
  });

  it('returns fail when p50 crosses fail threshold', async () => {
    const rows = Array.from({ length: 6 }, () => ({ latency_ms: 52830 }));
    const result = await exp.probe(makeCtx(rows));
  ...
  it('evidence exposes model + thresholds for operator audit', async () => {
    ...
    expect(ev.model).toBe('google/gemini-3.1-pro-preview');
    expect(ev.warn_threshold_ms).toBe(52230);
    expect(ev.fail_threshold_ms).toBe(52730);
  });
});
