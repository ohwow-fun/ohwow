...
describe('ToolchainToolTestAgentsExperiment (auto-generated)', () => {
  const exp = new ToolchainToolTestAgentsExperiment();
  ...
  it('returns pass when command exits 0', async () => {
    ...
    expect(ev.command).toBe('npx vitest run src/orchestrator/tools/__tests__/agents.test.ts');
  });
