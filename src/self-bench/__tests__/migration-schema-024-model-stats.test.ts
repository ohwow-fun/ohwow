...
const EXPECTED = ['ollama_model_snapshots', 'ollama_model_stats'];

describe('MigrationSchema024ModelStatsExperiment (auto-generated)', () => {
  const exp = new MigrationSchema024ModelStatsExperiment();
  ...
  it('evidence carries migration_file and the expected list', async () => {
    const rows = [{ name: 'ollama_model_snapshots' }];
    const result = await exp.probe(makeCtx(rows));
    const ev = result.evidence as { migration_file: string; expected_tables: string[] };
    expect(ev.migration_file).toBe('024-model-stats.sql');
