...
describe('ToolchainToolTestSynthesizeForGoalExperiment (auto-generated)', () => {
  const exp = new ToolchainToolTestSynthesizeForGoalExperiment();
  ...
  it('returns pass when command exits 0', async () => {
    ...
    expect(ev.command).toBe('npx vitest run src/orchestrator/tools/__tests__/synthesize-for-goal.test.ts');
  });
