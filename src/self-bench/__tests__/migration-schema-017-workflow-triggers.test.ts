...
const EXPECTED = ['agent_workforce_workflow_triggers'];

describe('MigrationSchema017WorkflowTriggersExperiment (auto-generated)', () => {
  const exp = new MigrationSchema017WorkflowTriggersExperiment();
  ...
  it('evidence carries migration_file and the expected list', async () => {
    const rows = [{ name: 'agent_workforce_workflow_triggers' }];
    const result = await exp.probe(makeCtx(rows));
    const ev = result.evidence as { migration_file: string; expected_tables: string[] };
    expect(ev.migration_file).toBe('017-workflow-triggers.sql');
