...
describe('ToolchainToolTestHumanGrowthExperiment (auto-generated)', () => {
  const exp = new ToolchainToolTestHumanGrowthExperiment();
  ...
  it('returns pass when command exits 0', async () => {
    ...
    expect(ev.command).toBe('npx vitest run src/orchestrator/tools/__tests__/human-growth.test.ts');
  });
