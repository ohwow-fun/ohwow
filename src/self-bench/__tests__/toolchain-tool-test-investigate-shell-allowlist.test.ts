...
describe('ToolchainToolTestInvestigateShellAllowlistExperiment (auto-generated)', () => {
  const exp = new ToolchainToolTestInvestigateShellAllowlistExperiment();
  ...
  it('returns pass when command exits 0', async () => {
    ...
    expect(ev.command).toBe('npx vitest run src/orchestrator/tools/__tests__/investigate-shell-allowlist.test.ts');
  });
