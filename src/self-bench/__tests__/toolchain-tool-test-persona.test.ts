...
import { ToolchainToolTestPersonaExperiment } from '../experiments/toolchain-tool-test-persona.js';
...
describe('ToolchainToolTestPersonaExperiment (auto-generated)', () => {
  const exp = new ToolchainToolTestPersonaExperiment();
  ...
  it('returns pass when command exits 0', async () => {
    ...
    expect(ev.command).toBe('npx vitest run src/orchestrator/tools/__tests__/persona.test.ts');
  });
