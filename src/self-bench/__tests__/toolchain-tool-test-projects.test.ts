...
import { ToolchainToolTestProjectsExperiment } from '../experiments/toolchain-tool-test-projects.js';
...
describe('ToolchainToolTestProjectsExperiment (auto-generated)', () => {
  const exp = new ToolchainToolTestProjectsExperiment();
  ...
  it('returns pass when command exits 0', async () => {
    ...
    expect(ev.command).toBe('npx vitest run src/orchestrator/tools/__tests__/projects.test.ts');
  });
