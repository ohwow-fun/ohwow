...
import { ToolchainToolTestOnboardingPlanExperiment } from '../experiments/toolchain-tool-test-onboarding-plan.js';
...
describe('ToolchainToolTestOnboardingPlanExperiment (auto-generated)', () => {
  const exp = new ToolchainToolTestOnboardingPlanExperiment();
  ...
  it('returns pass when command exits 0', async () => {
    ...
    expect(ev.command).toBe('npx vitest run src/orchestrator/tools/__tests__/onboarding-plan.test.ts');
  });
