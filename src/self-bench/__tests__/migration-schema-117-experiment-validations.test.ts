...
const EXPECTED = ['experiment_validations'];

describe('MigrationSchema117ExperimentValidationsExperiment (auto-generated)', () => {
  const exp = new MigrationSchema117ExperimentValidationsExperiment();
  ...
  it('evidence carries migration_file and the expected list', async () => {
    const rows = [{ name: 'experiment_validations' }];
    const result = await exp.probe(makeCtx(rows));
    const ev = result.evidence as { migration_file: string; expected_tables: string[] };
    expect(ev.migration_file).toBe('117-experiment-validations.sql');
