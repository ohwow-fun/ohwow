...
import { XiaomiMimoV2ProLatencyExperiment } from '../experiments/xiaomi-mimo-v2-pro-latency.js';
...
describe('XiaomiMimoV2ProLatencyExperiment (auto-generated)', () => {
  const exp = new XiaomiMimoV2ProLatencyExperiment();
  ...
  it('returns pass when p50 is below warn threshold', async () => {
    const rows = Array.from({ length: 50 }, () => ({ latency_ms: 12798 }));
    const result = await exp.probe(makeCtx(rows));
    expect(exp.judge(result, [])).toBe('pass');
  });

  it('returns warning when p50 crosses warn threshold', async () => {
    const rows = Array.from({ length: 50 }, () => ({ latency_ms: 25598 }));
    const result = await exp.probe(makeCtx(rows));
    const ev = result.evidence as { p50_latency_ms: number };
    expect(ev.p50_latency_ms).toBeGreaterThanOrEqual(25597);
    expect(exp.judge(result, [])).toBe('warning');
  });

  it('returns fail when p50 crosses fail threshold', async () => {
    const rows = Array.from({ length: 50 }, () => ({ latency_ms: 43897 }));
    const result = await exp.probe(makeCtx(rows));
  ...
  it('evidence exposes model + thresholds for operator audit', async () => {
    ...
    expect(ev.model).toBe('xiaomi/mimo-v2-pro');
    expect(ev.warn_threshold_ms).toBe(25597);
    expect(ev.fail_threshold_ms).toBe(43797);
  });
});
