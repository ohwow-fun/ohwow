...
const EXPECTED = ['agent_file_access_paths'];

describe('MigrationSchema023LocalFileAccessExperiment (auto-generated)', () => {
  const exp = new MigrationSchema023LocalFileAccessExperiment();
  ...
  it('evidence carries migration_file and the expected list', async () => {
    const rows = [{ name: 'agent_file_access_paths' }];
    const result = await exp.probe(makeCtx(rows));
    const ev = result.evidence as { migration_file: string; expected_tables: string[] };
    expect(ev.migration_file).toBe('023-local-file-access.sql');
