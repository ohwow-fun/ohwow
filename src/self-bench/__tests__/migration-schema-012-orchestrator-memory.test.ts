...
const EXPECTED = ['orchestrator_memory'];

describe('MigrationSchema012OrchestratorMemoryExperiment (auto-generated)', () => {
  const exp = new MigrationSchema012OrchestratorMemoryExperiment();
  ...
  it('evidence carries migration_file and the expected list', async () => {
    const rows = [{ name: 'orchestrator_memory' }];
    const result = await exp.probe(makeCtx(rows));
    const ev = result.evidence as { migration_file: string; expected_tables: string[] };
    expect(ev.migration_file).toBe('012-orchestrator-memory.sql');
