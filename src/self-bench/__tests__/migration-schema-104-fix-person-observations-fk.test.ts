...
const EXPECTED = ['agent_workforce_person_observations'];

describe('MigrationSchema104FixPersonObservationsFkExperiment (auto-generated)', () => {
  const exp = new MigrationSchema104FixPersonObservationsFkExperiment();
  ...
  it('evidence carries migration_file and the expected list', async () => {
    const rows = [{ name: 'agent_workforce_person_observations' }];
    const result = await exp.probe(makeCtx(rows));
    const ev = result.evidence as { migration_file: string; expected_tables: string[] };
    expect(ev.migration_file).toBe('104-fix-person-observations-fk.sql');
