...
describe('ToolchainToolTestStateExperiment (auto-generated)', () => {
  const exp = new ToolchainToolTestStateExperiment();
  ...
  it('returns pass when command exits 0', async () => {
    ...
    expect(ev.command).toBe('npx vitest run src/orchestrator/tools/__tests__/state.test.ts');
  });
