...
const EXPECTED = ['agent_workforce_attachments'];

describe('MigrationSchema015FileAttachmentsExperiment (auto-generated)', () => {
  const exp = new MigrationSchema015FileAttachmentsExperiment();
  ...
  it('evidence carries migration_file and the expected list', async () => {
    const rows = [{ name: 'agent_workforce_attachments' }];
    const result = await exp.probe(makeCtx(rows));
    const ev = result.evidence as { migration_file: string; expected_tables: string[] };
    expect(ev.migration_file).toBe('015-file-attachments.sql');
