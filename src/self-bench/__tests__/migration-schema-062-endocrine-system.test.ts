...
const EXPECTED = ['hormone_snapshots'];

describe('MigrationSchema062EndocrineSystemExperiment (auto-generated)', () => {
  const exp = new MigrationSchema062EndocrineSystemExperiment();
  ...
  it('evidence carries migration_file and the expected list', async () => {
    const rows = [{ name: 'hormone_snapshots' }];
    const result = await exp.probe(makeCtx(rows));
    const ev = result.evidence as { migration_file: string; expected_tables: string[] };
    expect(ev.migration_file).toBe('062-endocrine-system.sql');
