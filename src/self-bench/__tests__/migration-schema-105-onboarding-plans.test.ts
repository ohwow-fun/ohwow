...
const EXPECTED = ['agent_workforce_onboarding_plans'];

describe('MigrationSchema105OnboardingPlansExperiment (auto-generated)', () => {
  const exp = new MigrationSchema105OnboardingPlansExperiment();
  ...
  it('evidence carries migration_file and the expected list', async () => {
    const rows = [{ name: 'agent_workforce_onboarding_plans' }];
    const result = await exp.probe(makeCtx(rows));
    const ev = result.evidence as { migration_file: string; expected_tables: string[] };
    expect(ev.migration_file).toBe('105-onboarding-plans.sql');
