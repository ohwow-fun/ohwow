...
describe('ToolchainToolTestCollectiveIntelligenceExperiment (auto-generated)', () => {
  const exp = new ToolchainToolTestCollectiveIntelligenceExperiment();
  ...
  it('returns pass when command exits 0', async () => {
    ...
    expect(ev.command).toBe('npx vitest run src/orchestrator/tools/__tests__/collective-intelligence.test.ts');
  });
