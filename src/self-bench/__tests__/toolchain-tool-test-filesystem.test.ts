...
import { ToolchainToolTestFilesystemExperiment } from '../experiments/toolchain-tool-test-filesystem.js';
...
describe('ToolchainToolTestFilesystemExperiment (auto-generated)', () => {
  const exp = new ToolchainToolTestFilesystemExperiment();
  ...
  it('returns pass when command exits 0', async () => {
    ...
    expect(ev.command).toBe('npx vitest run src/orchestrator/tools/__tests__/filesystem.test.ts');
  });
