...
const EXPECTED = ['agent_workforce_person_models'];

describe('MigrationSchema103FixPersonModelsFkExperiment (auto-generated)', () => {
  const exp = new MigrationSchema103FixPersonModelsFkExperiment();
  ...
  it('evidence carries migration_file and the expected list', async () => {
    const rows = [{ name: 'agent_workforce_person_models' }];
    const result = await exp.probe(makeCtx(rows));
    const ev = result.evidence as { migration_file: string; expected_tables: string[] };
    expect(ev.migration_file).toBe('103-fix-person-models-fk.sql');
