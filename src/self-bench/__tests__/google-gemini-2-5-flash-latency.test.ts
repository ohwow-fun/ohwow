...
import { GoogleGemini25FlashLatencyExperiment } from '../experiments/google-gemini-2-5-flash-latency.js';
...
describe('GoogleGemini25FlashLatencyExperiment (auto-generated)', () => {
  const exp = new GoogleGemini25FlashLatencyExperiment();
  ...
  it('returns pass when p50 is below warn threshold', async () => {
    const rows = Array.from({ length: 50 }, () => ({ latency_ms: 8246 }));
    const result = await exp.probe(makeCtx(rows));
    expect(exp.judge(result, [])).toBe('pass');
  });

  it('returns warning when p50 crosses warn threshold', async () => {
    const rows = Array.from({ length: 50 }, () => ({ latency_ms: 16493 }));
    const result = await exp.probe(makeCtx(rows));
    const ev = result.evidence as { p50_latency_ms: number };
    expect(ev.p50_latency_ms).toBeGreaterThanOrEqual(16492);
    expect(exp.judge(result, [])).toBe('warning');
  });

  it('returns fail when p50 crosses fail threshold', async () => {
    const rows = Array.from({ length: 50 }, () => ({ latency_ms: 26667 }));
    const result = await exp.probe(makeCtx(rows));
  ...
  it('evidence exposes model + thresholds for operator audit', async () => {
    ...
    expect(ev.model).toBe('google/gemini-2.5-flash');
    expect(ev.warn_threshold_ms).toBe(16492);
    expect(ev.fail_threshold_ms).toBe(26567);
  });
});
