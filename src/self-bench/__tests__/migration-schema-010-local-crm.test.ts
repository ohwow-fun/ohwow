...
const EXPECTED = ['agent_workforce_contacts', 'agent_workforce_revenue_entries'];

describe('MigrationSchema010LocalCrmExperiment (auto-generated)', () => {
  const exp = new MigrationSchema010LocalCrmExperiment();
  ...
  it('evidence carries migration_file and the expected list', async () => {
    const rows = [{ name: 'agent_workforce_contacts' }];
    const result = await exp.probe(makeCtx(rows));
    const ev = result.evidence as { migration_file: string; expected_tables: string[] };
    expect(ev.migration_file).toBe('010-local-crm.sql');
