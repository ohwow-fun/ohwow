...
import { ToolchainToolTestResearchExperiment } from '../experiments/toolchain-tool-test-research.js';
...
describe('ToolchainToolTestResearchExperiment (auto-generated)', () => {
  const exp = new ToolchainToolTestResearchExperiment();
  ...
  it('returns pass when command exits 0', async () => {
    ...
    expect(ev.command).toBe('npx vitest run src/orchestrator/tools/__tests__/research.test.ts');
  });
