...
import { ToolchainToolTestCrmExperiment } from '../experiments/toolchain-tool-test-crm.js';
...
describe('ToolchainToolTestCrmExperiment (auto-generated)', () => {
  const exp = new ToolchainToolTestCrmExperiment();
  ...
  it('returns pass when command exits 0', async () => {
    ...
    expect(ev.command).toBe('npx vitest run src/orchestrator/tools/__tests__/crm.test.ts');
  });
