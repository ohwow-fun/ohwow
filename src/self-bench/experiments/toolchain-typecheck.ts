/**
 * TypeScript type checker health
 *
...
interface ToolchainTypecheckExperimentEvidence extends Record<string, unknown> {
  command: string;
  exit_code: number;
  stdout_lines: string[];
  stderr_lines: string[];
...
export class ToolchainTypecheckExperiment implements Experiment {
  id = 'toolchain-typecheck';
  name = 'TypeScript type checker health';
  category = 'tool_reliability' as const;
  hypothesis = 'npm run typecheck exits with code 0 on every run.';
  cadence = { everyMs: 1800000, runOnBoot: true };
  ...
  async probe(_ctx: ExperimentContext): Promise<ProbeResult> {
    ...
    if (!repoRoot) {
      const evidence: ToolchainTypecheckExperimentEvidence = {
        command,
        exit_code: -1,
        stdout_lines: [],
        stderr_lines: [],
      ...
      return {
        subject: 'subprocess:toolchain-typecheck',
        summary: `TypeScript type checker (npm run typecheck): repo root unavailable`,
        evidence,
      };
    }
    ...
    const summary =
      exitCode === 0
        ? `TypeScript type checker (npm run typecheck): passed in ${durationMs}ms`
        : `TypeScript type checker (npm run typecheck): failed (exit ${exitCode}) in ${durationMs}ms`;
  ...
  judge(result: ProbeResult, _history: Finding[]): Verdict {
    const ev = result.evidence as ToolchainTypecheckExperimentEvidence;
