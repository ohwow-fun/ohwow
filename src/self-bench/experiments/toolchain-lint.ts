...
interface ToolchainLintExperimentEvidence extends Record<string, unknown> {
  command: string;
  exit_code: number;
  stdout_lines: string[];
  stderr_lines: string[];
...
export class ToolchainLintExperiment implements Experiment {
  id = 'toolchain-lint';
  name = 'ESLint linter health';
  category = 'tool_reliability' as const;
  hypothesis = 'npm run lint exits with code 0 on every run.';
  cadence = { everyMs: 3600000, runOnBoot: true };
  ...
  async probe(_ctx: ExperimentContext): Promise<ProbeResult> {
    ...
    if (!repoRoot) {
      const evidence: ToolchainLintExperimentEvidence = {
        command,
        exit_code: -1,
        stdout_lines: [],
        stderr_lines: [],
      ...
      return {
        subject: 'subprocess:toolchain-lint',
        summary: `ESLint linter (npm run lint): repo root unavailable`,
        evidence,
      };
    }
    ...
    const summary =
      exitCode === 0
        ? `ESLint linter (npm run lint): passed in ${durationMs}ms`
        : `ESLint linter (npm run lint): failed (exit ${exitCode}) in ${durationMs}ms`;
  ...
  judge(result: ProbeResult, _history: Finding[]): Verdict {
    const ev = result.evidence as ToolchainLintExperimentEvidence;
