        ...
        if (useOllama) {
          ...
          while (iterations < MAX_TOOL_LOOP_ITERATIONS) {
            ...
            if (toolResults.length > 0) {
              ...
              if (detectStagnation(toolCallHashes) && toolResults.length > 0) {
    ...
    try {
      for (; iteration < MAX_TOOL_LOOP_ITERATIONS; iteration++) {
        ...
        for (const toolCall of response.toolCalls) {
          ...
          // Track tool call hash for stagnation detection
        ...
        // Inject stagnation warning into the last tool result message
