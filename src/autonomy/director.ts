  ...
  while (true) {
    ...
    try {
      ...
      await updatePhaseReport(db, {
        ...
        delta_ledger_json: JSON.stringify({ trios: 0, error: 'phase orchestrator threw' }),
        inbox_added_json: JSON.stringify({ count: 0 }),
    ...
    await updatePhaseReport(db, {
      ...
      cloud_sha_end,
      delta_pulse_json: buildDeltaPulse(pulseAtEntry, currentPulse),
      delta_ledger_json: JSON.stringify({ trios: phaseResult.trios.length }),
      inbox_added_json: JSON.stringify({ count: inbox_added }),
      remaining_scope:
