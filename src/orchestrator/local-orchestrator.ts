...
export class LocalOrchestrator {
  ...
  private async *runChat(
    ...
    const { staticPart, dynamicPart } = await buildTargetedPrompt(this.promptDeps, userMessage, sections, browserPreActivated || this.browserActivated, options?.platform);
    ...
    const tools = await this.getTools(options, browserPreActivated || this.browserActivated, sections);
    ...
    for (let iteration = 0; iteration < maxIter; iteration++) {
      ...
      for (let i = 0; i < outcomes.length; i++) {
        ...
        if (outcome.toolsModified && !this.browserActivated) {
          this.browserService = new LocalBrowserService({ headless: this.browserHeadless });
          this.browserActivated = true;
        ...
        // Anthropic format: use formattedBlocks (with images) for browser results
        toolResults.push({
          type: 'tool_result',
  ...
  private async *runOllamaToolLoop(
    ...
    const { staticPart: ollamaStatic, dynamicPart: ollamaDynamic } = await buildTargetedPrompt(this.promptDeps, userMessage, classified.sections, browserPreActivated || this.browserActivated, options?.platform);
    const systemPrompt = ollamaStatic + '\n\n' + ollamaDynamic;

    // Convert Anthropic tool definitions to OpenAI format
    const anthropicTools = await this.getTools(options, browserPreActivated || this.browserActivated, classified.sections);
