 ...
 *
 * Tool schema definitions for the whole agent + task management cluster
 * (list_agents through cancel_task) live at the top so the orchestrator
 * catalog can spread them in source order.
 */
...
export const AGENT_TASK_TOOL_DEFINITIONS: Tool[] = [
  {
    name: 'list_agents',
    description:
...
// ============================================================================
// PROCEDURE-SKILL SOP MATCHER — REMOVED
// ============================================================================
//
// This file used to carry `sopToSequence()` + a keyword-matching block
// inside `runAgent` that intercepted the user's prompt, looked up
// active `skill_type='procedure'` rows in `agent_workforce_skills`,
// ran `matchesTriggers(skill.triggers, extractKeywords(prompt))`, and
// either decomposed the matched skill's tool_sequence into a 5-phase
// sequence or built a 300-line desktop-SOP prompt from it.
//
// That path is gone. Rationale and full removal plan at:
//   /Users/jesus/.claude/plans/idempotent-tumbling-flame.md
//
// Short version: the matcher was keyword-based, not LLM-semantic,
// and routinely over-matched. A prompt to rewrite local `.md` files
// matched the word "write" and routed into an X-compose sub-agent
// against an unauth'd Chrome. LLM tool selection from the prompt's
// tool list is strictly better than surface-feature keyword regex,
// and the 3 active procedure rows were all degenerate single-tool
// wrappers that add nothing over the LLM picking those tools
// directly. They've been archived via migration 108.
//
// Other matchers with the same bug have been removed in lockstep:
//   - `engine.ts:compileSkills()` (gutted to early-return '')
//   - `prompt-builder.ts` triggerMatched block (deleted)
//
// `trackSkillUsage()` in local-orchestrator.ts was also a procedure-
// specific EMA rolling-average updater — deleted. Code skills have
// their own metrics loop in `runtime-skill-metrics.ts`.
...
export async function runAgent(
  ...
  // NOTE: the procedure-skill keyword SOP matcher used to live here.
  // It's been removed — see the banner at the top of this file and
  // /Users/jesus/.claude/plans/idempotent-tumbling-flame.md. The
  // prompt now flows straight through to the sub-agent without any
  // keyword hijacking. Discovery of skills is the LLM's job,
  // operating on its tool list, not a regex on the user's message.
