...
export function classifyIntent(message: string, previousIntent?: ClassifiedIntent): ClassifiedIntent {
  ...
  // Desktop control (must be before browser — "screenshot" overlaps)
  if (/\b(desktop|screen|macOS|mac os|my computer|click on|move.*mouse|type in)\b/.test(lower) &&
      !/\b(website|url|http|www|browse|web\s*page)\b/.test(lower)) {
    return classify('desktop', new Set<IntentSection>(['desktop', 'memory']), 'Preparing desktop control...', lower);
  }
  if (/\b(screenshot)\b/.test(lower) && /\b(desktop|screen|my)\b/.test(lower) &&
      !/\b(website|url|http|www|browse|web\s*page)\b/.test(lower)) {
    return classify('desktop', new Set<IntentSection>(['desktop', 'memory']), 'Preparing desktop control...', lower);
  }
