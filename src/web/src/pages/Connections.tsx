...
import { PlugsConnected, Plus, MagnifyingGlass, Trash, ArrowClockwise, Eye, CircleNotch } from '@phosphor-icons/react';
...
export function ConnectionsPage() {
  ...
  return (
    <div className="p-6 max-w-4xl">
      <PageHeader
        title="Connections"
        subtitle="A2A agent connections"
        action={
          <button
      ...
      {loading ? (
        ...
        <EmptyState
          ...
          description={search ? `No connections matching "${search}".` : 'Connect to external A2A agents.'}
        />
      ) : (
            ...
            <div key={conn.id} className="flex items-center justify-between px-4 py-3">
              ...
              >
                ...
                <p className="text-xs text-neutral-400 truncate">
                  ...
                  {conn.auth_type !== 'none' && (
                    <span className="ml-2">· {conn.auth_type.replace(/_/g, ' ')}</span>
                  )}
      ...
      {/* Create form */}
      {showForm && (
        <NewConnectionForm onClose={() => setShowForm(false)} onSuccess={() => { refetch(); setShowForm(false); }} />
      )}
...
function NewConnectionForm({ onClose, onSuccess }: { onClose: () => void; onSuccess: () => void }) {
  ...
  return (
    <Modal open onClose={onClose} title="New connection" maxWidth="max-w-lg">
