...
export function Dashboard({ config, db, rawDb, needsOnboarding, onStartOnboarding, onConfigChange }: DashboardProps) {
  ...
  // Model picker state (moved from ChatScreen)
  const [showModelPicker, setShowModelPicker] = useState(false);
  const [availableModels, setAvailableModels] = useState<string[]>([]);
  const [modelsLoaded, setModelsLoaded] = useState(false);
  const [modelPickerError, setModelPickerError] = useState<string | null>(null);
  const [pickerIdx, setPickerIdx] = useState(0);
  const [cloudModelIds, setCloudModelIds] = useState<Set<string>>(new Set());
  const [pickerStep, setPickerStep] = useState<'source' | 'models' | 'cloud_auth'>('source');
  const [pickerSource, setPickerSource] = useState<'cloud' | 'local'>('local');
  const [pickerApiKey, setPickerApiKey] = useState('');
  const [pickerApiKeyValidating, setPickerApiKeyValidating] = useState(false);
  const [pickerApiKeyError, setPickerApiKeyError] = useState('');
  ...
  // Load available models when picker is opened
  useEffect(() => {
    ...
    const load = async () => {
      ...
      try {
        ...
        if (config.anthropicApiKey || config.anthropicOAuthToken) {
          const cloudModels = ['claude-haiku-4-5-20251001', 'claude-sonnet-4-20250514'];
          for (const cm of cloudModels) {
            models.push(cm);
            cloudIds.add(cm);
          }
        }
        const res = await fetch(`http://127.0.0.1:${config.port}/api/ollama/models`, {
          headers: runtime.sessionToken ? { Authorization: `Bearer ${runtime.sessionToken}` } : {},
          signal: AbortSignal.timeout(10000),
        });
        if (!res.ok) throw new Error(`HTTP ${res.status}`);
        const json = await res.json() as {
          data: Array<{ modelName: string; status: string }>;
        };
        for (const m of json.data) {
          if (!models.includes(m.modelName)) models.push(m.modelName);
        }
        if (!cancelled) {
          setAvailableModels(models);
          setCloudModelIds(cloudIds);
          setModelsLoaded(true);

          // Always start at source step
          const currentIsCloud = cloudIds.has(orchestrator.currentModel);
          setPickerSource(currentIsCloud ? 'cloud' : 'local');
          setPickerIdx(currentIsCloud ? 0 : 1);
          setPickerStep('source');
        }
      } catch {
        if (!cancelled) setModelPickerError("Couldn't load models. Press Esc.");
      }
    };
    load();
    return () => { cancelled = true; };
  }, [showModelPicker, config.anthropicApiKey, config.anthropicOAuthToken, config.port, runtime.sessionToken, orchestrator.currentModel]);
  ...
  useInput((input, key) => {
    ...
    // Model picker intercepts all input
    if (showModelPicker) {
      if (pickerStep === 'source') {
        if (key.escape) { setShowModelPicker(false); return; }
        if (input === 'j' || key.downArrow) { setPickerIdx(i => Math.min(i + 1, 1)); return; }
        if (input === 'k' || key.upArrow) { setPickerIdx(i => Math.max(i - 1, 0)); return; }
        if (key.return) {
          const source = pickerIdx === 0 ? 'cloud' : 'local';
          if (source === 'cloud' && cloudModelIds.size === 0) {
            // No API key configured, show auth method chooser
            setPickerStep('cloud_auth');
            setPickerApiKeyError('');
            return;
          }
          const filtered = source === 'cloud'
            ? availableModels.filter(m => cloudModelIds.has(m))
            : availableModels.filter(m => !cloudModelIds.has(m));
      ...
      if (pickerStep === 'cloud_auth') {
        ...
        if (pickerApiKeyValidating) return; // ignore input while validating
        if (key.backspace || key.delete) {
          setPickerApiKey(k => k.slice(0, -1));
          setPickerApiKeyError('');
          return;
        }
        if (key.return) {
          if (!pickerApiKey.trim()) return;
          setPickerApiKeyValidating(true);
          setPickerApiKeyError('');
          validateAnthropicApiKey(pickerApiKey.trim()).then(valid => {
            setPickerApiKeyValidating(false);
            if (valid) {
              const trimmedKey = pickerApiKey.trim();
              updateConfigFile({ anthropicApiKey: trimmedKey });
              onConfigChange?.({ ...config, anthropicApiKey: trimmedKey });
              // Notify running daemon so it switches to Anthropic path immediately
              ...
              // Add cloud models directly
              const cloudModels = ['claude-haiku-4-5-20251001', 'claude-sonnet-4-20250514'];
              const newCloudIds = new Set(cloudModels);
              setCloudModelIds(newCloudIds);
              setAvailableModels(prev => {
                const merged = [...cloudModels];
                for (const m of prev) {
                  if (!newCloudIds.has(m)) merged.push(m);
                }
                return merged;
              });
              setPickerSource('cloud');
              setPickerStep('models');
              setPickerIdx(0);
              setPickerApiKey('');
            } else {
              setPickerApiKeyError('Invalid API key. Check and try again.');
            }
          });
          return;
        }
        if (input && !key.ctrl && !key.meta) {
          setPickerApiKey(k => k + input);
          setPickerApiKeyError('');
        }
        return;
      }
      ...
      // pickerStep === 'models'
      const filteredModels = pickerSource === 'cloud'
        ? availableModels.filter(m => cloudModelIds.has(m))
        : availableModels.filter(m => !cloudModelIds.has(m));

      if (key.escape) {
        // Always go back to source step
        setPickerStep('source');
        setPickerIdx(pickerSource === 'cloud' ? 0 : 1);
        return;
      }
      if (filteredModels.length > 0) {
        if (input === 'j' || key.downArrow) { setPickerIdx(i => Math.min(i + 1, filteredModels.length - 1)); return; }
        if (input === 'k' || key.upArrow) { setPickerIdx(i => Math.max(i - 1, 0)); return; }
        if (key.return) {
          const selected = filteredModels[pickerIdx];
          if (selected) {
            const source = cloudModelIds.has(selected) ? 'cloud' as const : 'local' as const;
            if (source === 'cloud') {
              updateConfigFile({ modelSource: 'cloud', cloudModel: selected });
              onConfigChange?.({ ...config, modelSource: 'cloud', cloudModel: selected });
            } else {
            ...
            fetch(`http://127.0.0.1:${config.port}/api/models/orchestrator`, {
              ...
              body: JSON.stringify({ model: selected, modelSource: source }),
            }).catch(() => {});
            orchestrator.setModel(selected, source);
    ...
    if (isHome && focusZone === 'chat') {
      ...
      // 's' shortcut for session picker (when not streaming and not typing)
      if (input === 's' && !orchestrator.isStreaming && !inputValue) {
        openScreen(Screen.Sessions);
        return;
      }
  ...
  const renderModelPicker = () => {
    ...
    const cloudCount = availableModels.filter(m => cloudModelIds.has(m)).length;
    const localCount = availableModels.filter(m => !cloudModelIds.has(m)).length;

    if (pickerStep === 'source' && modelsLoaded) {
      const sources = [
        { key: 'cloud' as const, icon: '\u2601', label: 'Cloud', count: cloudCount, color: 'magenta' as const },
        { key: 'local' as const, icon: '\u2299', label: 'Local', count: localCount, color: 'cyan' as const },
      ];
      return (
        <Box flexDirection="column" borderStyle="round" borderColor="cyan" paddingX={2} paddingY={1}>
          <Text bold>Switch Model</Text>
          <Text color="gray" dimColor>Current: {orchestrator.currentModel}</Text>
          <Box marginTop={1} flexDirection="column">
            {sources.map((s, i) => {
              const isSelected = i === pickerIdx;
              const countLabel = s.count === 0 && s.key === 'cloud' ? 'set up' : String(s.count);
              return (
                <Text key={s.key}>
                  <Text color={isSelected ? 'cyan' : 'white'}>{isSelected ? '\u276F ' : '  '}</Text>
                  <Text color={isSelected ? 'cyan' : s.color}>
                    {s.icon} {s.label} ({countLabel})
                  </Text>
                </Text>
              );
            })}
    ...
    if (pickerStep === 'cloud_auth') {
      const masked = pickerApiKey.length <= 8
        ? '\u2022'.repeat(pickerApiKey.length)
        : pickerApiKey.slice(0, 7) + '\u2022'.repeat(Math.min(pickerApiKey.length - 7, 20));
      return (
        <Box flexDirection="column" borderStyle="round" borderColor="cyan" paddingX={2} paddingY={1}>
          ...
          {pickerApiKeyError !== '' && (
            <Box marginTop={1}>
              <Text color="red">{pickerApiKeyError}</Text>
            </Box>
          )}
          <Box marginTop={1}>
    ...
    // Models step (or loading/error states)
    const filteredModels = pickerSource === 'cloud'
      ? availableModels.filter(m => cloudModelIds.has(m))
      : availableModels.filter(m => !cloudModelIds.has(m));
    const sourceIcon = pickerSource === 'cloud' ? '\u2601' : '\u2299';
    const sourceLabel = pickerSource === 'cloud' ? 'Cloud' : 'Local';
    const sourceColor = pickerSource === 'cloud' ? 'magenta' : 'cyan';
    return (
      <Box flexDirection="column" borderStyle="round" borderColor="cyan" paddingX={2} paddingY={1}>
        <Text bold>Switch Model  <Text color={sourceColor}>{sourceIcon} {sourceLabel}</Text></Text>
        <Text color="gray" dimColor>Current: {orchestrator.currentModel}</Text>
        <Box marginTop={1} flexDirection="column">
          {modelPickerError ? (
            ...
            <Text color="gray">No {sourceLabel.toLowerCase()} models available.</Text>
          ) : (
            filteredModels.map((m, i) => {
              const isCloud = cloudModelIds.has(m);
              const catalogEntry = MODEL_CATALOG.find(c => m === c.tag || m.startsWith(`${c.tag}:`));
              const noTools = catalogEntry && !catalogEntry.toolCalling;
              const isActive = m === orchestrator.currentModel;
              const isSelected = i === pickerIdx;
              let label = m;
              if (isActive) label += ' (active)';
              if (noTools) label += ' (no tools)';
              const prefix = isCloud ? '\u2601' : '\u2299';
              const modelColor = isSelected ? 'cyan' : isActive ? 'green' : isCloud ? 'magenta' : 'white';
              return (
                <Text key={m}>
                  <Text color={isSelected ? 'cyan' : 'white'}>{isSelected ? '\u276F ' : '  '}</Text>
                  <Text color={modelColor}>{prefix} {label}</Text>
                </Text>
              );
            })
          )}
