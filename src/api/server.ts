...
export function createServer(deps: ServerDeps): {
  ...
  const { config, db, rawDb, startTime, eventBus, engine, orchestrator, sessionToken, triggerEvaluator, workspaceId, voiceboxService, modelRouter, getWhatsAppClient, channelRegistry, messageRouter, onScheduleChange } = deps;
  const app = express();
