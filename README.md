...
ohwow works as a Claude Code plugin. 25 tools become available directly in your Claude conversations.
...
Or set up via the CLI:

```bash
ohwow setup-claude-code
```

Or add manually to any project's `.mcp.json`:
...
|---|---|---|
| Core | `ohwow_chat`, `ohwow_run_agent`, `ohwow_list_agents`, `ohwow_get_task`, `ohwow_list_tasks`, `ohwow_workspace_status` | Talk to the orchestrator, run agents, track tasks |
| CRM | `ohwow_list_contacts`, `ohwow_create_contact`, `ohwow_search_contacts` | Manage your contact pipeline |
| Workflows | `ohwow_list_workflows`, `ohwow_run_workflow`, `ohwow_list_automations`, `ohwow_run_automation` | Execute and manage automations |
| Projects | `ohwow_list_projects`, `ohwow_create_project`, `ohwow_list_goals` | Organize work and track goals |
| Knowledge | `ohwow_list_knowledge`, `ohwow_search_knowledge`, `ohwow_add_knowledge_url` | Build and query your knowledge base |
| Research | `ohwow_deep_research`, `ohwow_scrape_url` | Web research and scraping |
| Messaging | `ohwow_send_message`, `ohwow_list_chats` | WhatsApp and Telegram |
| Cloud | `ohwow_list_sites`, `ohwow_list_integrations` | Dashboard sites and connected services |

**Example from Claude Code:**

```
You: "Research the top 5 competitors in our space, add the findings to the
     knowledge base, then create a contact for each CEO"

Claude uses: ohwow_deep_research → ohwow_add_knowledge_url → ohwow_create_contact (x5)
```

The ohwow daemon must be running for tools to work. Start it with `ohwow` or `ohwow --daemon`.
